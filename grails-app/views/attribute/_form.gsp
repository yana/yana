...
<div class="fieldcontain ${hasErrors(bean: attributeInstance, field: 'filter', 'error')} required">
	...
	<span class="styled-select"><g:select id="filter" name="filter.id" from="${org.yana.Filter.list()}" optionKey="id" required="" value="${attributeInstance?.filter?.id}" class="many-to-one"/></span>
</div>
