...
<div class="fieldcontain ${hasErrors(bean: nodeTypeRelationshipInstance, field: 'parent', 'error')} required">
	...
	<span class="styled-select"><g:select id="parent" name="parent.id" from="${org.yana.NodeType.list()}" optionKey="id" required="" value="${(params.parent)?params.parent.id:nodeTypeRelationshipInstance?.parent?.id}" class="many-to-one"/></span>
</div>
...
<div class="fieldcontain ${hasErrors(bean: nodeTypeRelationshipInstance, field: 'child', 'error')} required">
	...
	<span class="styled-select"><g:select id="child" name="child.id" from="${org.yana.NodeType.list()}" optionKey="id" required="" value="${(params.child)?params.child.id:nodeTypeRelationshipInstance?.child?.id}" class="many-to-one"/></span>
</div>
